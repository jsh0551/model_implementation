    ...
    def __init__(self, in_channel, out_channel, start_stride = 1, bottle_neck = False, reduction = False):
        ...
        if not bottle_neck:
            self.block = nn.Sequential(nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=start_stride, padding=1),
                        ...
                        nn.ReLU(),
                        nn.Sequential(nn.Conv2d(out_channel, out_channel, kernel_size=3, padding=1),
                        nn.BatchNorm2d(out_channel),
                        nn.ReLU())
                        )
            ...
            self.block = nn.Sequential(nn.Conv2d(self.scale*out_channel, out_channel, kernel_size=1, stride=start_stride),
                        ...
                        nn.BatchNorm2d(out_channel),
                        nn.ReLU()),
                        nn.Sequential(nn.Conv2d(out_channel, 4*out_channel, kernel_size=1),
                        nn.BatchNorm2d(out_channel),
                        nn.ReLU())
                        )
...
class ResNet(nn.Module):
    def __init__(self, num_layers = 34, num_blocks = [3,4,6,3], num_classes = 10, bottle_neck = False):
        ...
        self.block2 = block(64, 64, bottle_neck = bottle_neck)
        self.block3 = block(128, 128, bottle_neck = bottle_neck)
        self.block4 = block(256, 256, bottle_neck = bottle_neck)
        self.block5 = block(512, 512, bottle_neck = bottle_neck)

        self.conv_layer2 = nn.Sequential(*[self.block2 for _ in range(num_blocks[0])])

        self.conv_layer3_start = block(64, 128, start_stride=2, bottle_neck = bottle_neck, reduction = True)
        self.conv_layer3 = nn.Sequential(*[self.block3 for _ in range(num_blocks[1]-1)])

        self.conv_layer4_start = block(128, 256, start_stride=2, bottle_neck = bottle_neck, reduction = True)
        self.conv_layer4 = nn.Sequential(*[self.block4 for _ in range(num_blocks[2]-1)])

        self.conv_layer5_start = block(256, 512, start_stride=2, bottle_neck = bottle_neck, reduction = True)
        self.conv_layer5 = nn.Sequential(*[self.block5 for _ in range(num_blocks[3]-1)])
